...
export const SnapchatPhotoMap: React.FC<SnapchatPhotoMapProps> = ({
  ...
  const filteredIncidents = activeFilter === 'all'
    ? SNAP_PHOTO_INCIDENTS
    : SNAP_PHOTO_INCIDENTS.filter(inc => inc.problemType === activeFilter);
      ...
      <div className="bg-slate-900/90 backdrop-blur-md p-2 rounded-2xl border border-slate-700/80 shadow-md flex items-center justify-between gap-2 overflow-x-auto scrollbar-none">
        ...
        <div className="hidden sm:flex items-center gap-1 bg-slate-800 p-0.5 rounded-xl border border-slate-700 shrink-0 text-[11px] font-bold">
          <button
            ...
            className={`px-2.5 py-1 rounded-lg transition-colors ${mapTileStyle === 'snapDark' ? 'bg-amber-400 text-slate-950' : 'text-slate-300 hover:text-white'}`}
          >
            Snap Dark
          </button>
          <button
            onClick={() => setMapTileStyle('satellite')}
            className={`px-2.5 py-1 rounded-lg transition-colors ${mapTileStyle === 'satellite' ? 'bg-amber-400 text-slate-950' : 'text-slate-300 hover:text-white'}`}
          >
            Satellite
          </button>
          <button
            onClick={() => setMapTileStyle('street')}
            className={`px-2.5 py-1 rounded-lg transition-colors ${mapTileStyle === 'street' ? 'bg-amber-400 text-slate-950' : 'text-slate-300 hover:text-white'}`}
          >
            Streets
          </button>
        </div>
        ...
        <div className="absolute top-3 left-3 z-10 bg-black/75 backdrop-blur-md text-white px-3.5 py-2 rounded-2xl border border-white/15 shadow-lg flex items-center gap-3 text-xs">
          <div className="flex items-center gap-1.5">
            ...
            <span className="font-bold">
              {filteredIncidents.length} {activeFilter === 'all' ? 'Civic Incidents' : `${activeFilter.toUpperCase()} Incidents`} Live
            </span>
          </div>
          <span className="text-slate-400 hidden sm:inline">| Tap any photo story bubble to inspect & corroborate</span>
        </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-sm font-bold text-white flex items-center gap-1.5">
                    ...
                    <span className="px-1.5 py-0.2 rounded-md bg-red-600 text-[9px] font-black uppercase text-white">
                      Live Snap
                    </span>
                  </div>
