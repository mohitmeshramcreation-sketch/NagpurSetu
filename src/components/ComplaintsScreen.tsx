...
import { CaseItem, ComplaintCategory, Department } from '../types';
  ...
  const handleSubmitComplaint = (e: React.FormEvent) => {
    ...
    const newCase: CaseItem = {
      ...
      timeline: [
        ...
        {
          id: `tl-4`,
          title: 'Resolution & Citizen Verification',
          timestamp: 'Pending',
          status: 'pending',
          dotColor: 'gray'
        }
              ...
              <form onSubmit={handleSubmitComplaint} className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 space-y-6 shadow-xs">
                ...
                <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                  ...
                  <button
                    ...
                    className="px-6 py-3 bg-emerald-700 text-white font-bold text-xs rounded-xl hover:bg-emerald-800 transition-colors disabled:opacity-50 flex items-center gap-2 shadow-sm cursor-pointer"
                  >
                    <Send className="w-4 h-4" />
                    <span>{isSubmitting ? 'Submitting...' : 'Submit Complaint & Generate Reference ID'}</span>
                  </button>
                </div>
              </form>
            )}
